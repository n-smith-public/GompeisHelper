...
public class ModdingCommands extends ListenerAdapter {
        ...
        if (event.getName().equals("modding")) {
            ...
            switch (userInput) {
                case "user_directory":
                    ...
                    eb.setTitle(a.build().getTitle());
                    eb.setDescription(a.build().getDescription());
                    break;
                case "localisation":
                    EmbedBuilder b = localisation();
                    eb.setTitle(b.build().getTitle());
                    eb.setDescription(b.build().getDescription());
                    break;
                case "debug":
                    EmbedBuilder c = debug();
                    eb.setTitle(c.build().getTitle());
                    eb.setDescription(c.build().getDescription());
                    break;
                case "workshop":
                    EmbedBuilder d = workshop();
                    eb.setTitle(d.build().getTitle());
                    eb.setDescription(d.build().getDescription());
                    break;
                case "crash_data_log":
                    EmbedBuilder e = crash_data_log();
                    eb.setTitle(e.build().getTitle());
                    eb.setDescription(e.build().getDescription());
                    break;
                case "search_in_files":
                    EmbedBuilder f = search_in_files();
                    eb.setTitle(f.build().getTitle());
                    eb.setDescription(f.build().getDescription());
                    break;
                case "crash_mainmenu":
                    EmbedBuilder g = crash_mainmenu();
                    eb.setTitle(g.build().getTitle());
                    eb.setDescription(g.build().getDescription());
                    break;
                case "crash_country":
                    EmbedBuilder h = crash_country();
                    eb.setTitle(h.build().getTitle());
                    eb.setDescription(h.build().getDescription());
                    break;
                case "client_ping":
                    EmbedBuilder i = client_ping();
                    eb.setTitle(i.build().getTitle());
                    eb.setDescription(i.build().getDescription());
                    break;
                case "unexpected_token":
                    EmbedBuilder j = unexpected_token();
                    eb.setTitle(j.build().getTitle());
                    eb.setDescription(j.build().getDescription());
                    break;
                case "nudge_states":
                    EmbedBuilder k = nudge_states();
                    eb.setTitle(k.build().getTitle());
                    eb.setDescription(k.build().getDescription());
                    break;
                case "githelp":
                    EmbedBuilder l = githelp();
                    eb.setTitle(l.build().getTitle());
                    eb.setDescription(l.build().getDescription());
                    break;
                case "mod_error":
                    EmbedBuilder m = mod_error();
                    eb.setTitle(m.build().getTitle());
                    eb.setDescription(m.build().getDescription());
                    break;
                case "date_event":
                    EmbedBuilder n = date_event();
                    eb.setTitle(n.build().getTitle());
                    eb.setDescription(n.build().getDescription());
                    break;
                case "mapmodding":
                    EmbedBuilder o = mapmodding();
                    eb.setTitle(o.build().getTitle());
                    eb.setImage(String.valueOf(o.build().getImage()));
                    eb.setDescription(o.build().getDescription());
                    break;
                case "hoi4moddingedge":
                    EmbedBuilder p = hoi4moddingedge();
                    eb.setTitle(p.build().getTitle());
                    eb.setDescription(p.build().getDescription());
                    break;
                case "shine":
                    EmbedBuilder q = shine();
                    eb.setTitle(q.build().getTitle());
                    eb.setDescription(q.build().getDescription());
                    break;
                default:
    ...
    private EmbedBuilder localisation() {
        EmbedBuilder value = new EmbedBuilder();
        value.setTitle("Localisation");
        value.setDescription("Most localisation-related errors appear from one of the 6 possible causes.\n" +
    ...
    private EmbedBuilder debug() {
        EmbedBuilder value = new EmbedBuilder();
        value.setTitle("Debugging");
        value.setDescription("The debug mode has two ways to be enabled in launch options:\n" +
    ...
    private EmbedBuilder crash_mainmenu() {
        EmbedBuilder value = new EmbedBuilder();
        value.setTitle("Crash in Main Menu");
        value.setDescription("These are some common crashes that can happen during the main menu loading, identified by the last read file as stated by the crash data log (see `!crash_data_log`) and the portion of the main menu they're crashing on.\n" +
                "\n" +
    ...
    private EmbedBuilder crash_country() {
        EmbedBuilder value = new EmbedBuilder();
        value.setTitle("Crash in Country Selection");
        value.setDescription("These are some common crashes during the country selection screen, identified by the last read file as stated by the crash data log (see `!crash_data_log`) and the part of the selection process that they crash on. Most of these are after selecting a country during the loading.\n" +
                "\n" +
    ...
    private EmbedBuilder client_ping() {
        EmbedBuilder value = new EmbedBuilder();
        value.setTitle("Client Ping");
        value.setDescription("These are some common crashes marked with either `client_ping` or `hourly_tick` as the last read script within the crash data log (see `!crash_data_log`), and occur in the middle of the game only if the AI is turned on.\n" +
                "\n" +
    ...
    private EmbedBuilder nudge_states() {
        EmbedBuilder value = new EmbedBuilder();
        value.setTitle("Nudge States");
        value.setDescription("This is the process of making states using the nudge:\n" +
                "\n" +
    ...
    private EmbedBuilder githelp() {
        EmbedBuilder value = new EmbedBuilder();
        value.setTitle("Git Help");
        value.setDescription("Github and Gitlab are services that provide storing an online copy of the files, also allowing collaboration between people. Using it is the best way to work as a team, also allowing storing backups of the mod in case some data gets lost or a disasterous error appears in a change.\n" +
                "\n" +
    ...
    private EmbedBuilder mod_error() {
        EmbedBuilder value = new EmbedBuilder();
        value.setTitle("Mod Error");
        value.setDescription("The game reads mods through .mod files located directly within the user directory's (See `!user_directory`) `/mod/` folder, opened via a text editor. If it doesn't exist, the game would have no idea where to find the mod files.\n" +
    ...
    private EmbedBuilder date_event() {
        EmbedBuilder value = new EmbedBuilder();
        value.setTitle("Event on X Date");
        value.setDescription("In order to fire an event on a specific date or date range, the event should be fired on startup with a delay, **with automatic firing disabled**, similarly to a event fired from a focus or another effect. If there are additional conditions, then `trigger = { ... }` within the event will check for it. Remember that leap days do not exist in-game.\n" +
    ...
    private EmbedBuilder mapmodding() {
        EmbedBuilder value = new EmbedBuilder();
        value.setTitle("Map Modding Guidelines");
        value.setImage("https://cdn.discordapp.com/attachments/1126589959680311366/1126638168586854430/f6e16879a37ad98e.png");
        value.setDescription("Before making a custom map, look at the flow chart");
        return value;
    }

    private EmbedBuilder hoi4moddingedge() {
        EmbedBuilder value = new EmbedBuilder();
        value.setTitle("HOI4 Modding Edge");
        value.setDescription("Hello, you mentioned the website https://www.hoi4modding.com in your post. Please be aware that this tool is not recommended by our moderation staff and other experienced modders. The code output from this website tends to result in numerous bugs, leaving more posts on here than if you had just learned to make focus trees properly. The site also discourages you from learning how to make focus trees properly, and the changes take much longer to test compared to them instantly being loaded in-game once the file is saved by the -debug launch option.\n" +
                "\n" +
